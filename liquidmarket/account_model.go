...
func (a *Account) UpdateAccount(db *sql.DB, accountUuid uuid.UUID) error {
	statement := fmt.Sprintf("CALL account_update('%s', unhex(replace('%s', '-', '')))", a.Name, accountUuid)
	_, err := db.Exec(statement)
	return err
}
func DeleteAccount(db *sql.DB, accountUuid uuid.UUID) error {
	statement := fmt.Sprintf("CALL account_delete(unhex(replace('%s', '-', '')))", accountUuid)
	_, err := db.Exec(statement)
	return err
}
func (a *Account) CreateAccount(db *sql.DB, googleId string) error {
	statement := fmt.Sprintf("CALL account_create('%s', '%s');", a.Name, googleId)
	_, err := db.Exec(statement)
	if err != nil {
		return err
	}
