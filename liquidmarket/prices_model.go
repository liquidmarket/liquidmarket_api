...
func GetListingsWithPrices(db *sql.DB) ([]Listing, error) {
	...
	for rows.Next() {
		var m Listing
		if err := rows.Scan(&m.ID, &m.OfferID, &m.OrganisationName, &m.ShortName, &m.MarketMakerID, &m.MarketMakerName, &m.TotalShares, &m.BuyPrice, &m.SellPrice, &m.Price, &m.Spread); err != nil {
			return nil, err
		}
		priceQuery := fmt.Sprintf("SELECT marketmaker_id, id, price, spread, offered_at FROM offers WHERE market_id = %d ORDER BY offered_at DESC LIMIT 30", m.ID)
		////
		...
		for priceQueryRows.Next() {
			...
			if err := priceQueryRows.Scan(&p.MarketMakerID, &p.ID, &p.Price, &p.Spread, &timeByteArray); err != nil {
				return nil, err
			}
